...
function renderPreview(reset=false){
  ...
  r.min=b.min;r.max=b.max;r.step=1;
...
function fitPreview(){
  ...
  const designWidth=isMobile?390:1180;
  ...
  frame.style.height=`${Math.max(isMobile?760:620,height+32)}px`;
}
...
function money(v){
  try{return new Intl.NumberFormat(LOCALE,{style:"currency",currency:CURRENCY,maximumFractionDigits:0}).format(Number(v)||0);}
  catch{return (Number(v)||0).toLocaleString(LOCALE)+" "+CURRENCY;}
}
function fmt(v){
  return Number.isInteger(Number(v))?String(v):String(v).replace(".",",");
}
function fitStatValue(el){
  if(!el)return;
  el.style.fontSize="";
  const maxSize=32,minSize=16;
  let size=maxSize;
...
minus.onclick=()=>{range.value=Math.max(+range.min,+range.value-1);update()};plus.onclick=()=>{range.value=Math.min(+range.max,+range.value+1);update()};range.oninput=update;
