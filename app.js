...
const PRESETS = {
  CZ:{locale:"cs-CZ",currency:"CZK",xlsxFilename:"Calculator_Data_CZ.xlsx",
    texts:{title:"Práce navíc, kde si výši výdělku určujete sami",subtitle:"Spočítejte si orientační měsíční provizi podle počtu klientů.",client_question:"Kolik klientů obsloužíte za měsíc?",badge:"ORIENTAČNÍ VÝPOČET",commission_title:"Odhad provize",monthly_commission:"měsíční provize",clients_label:"Počet klientů",time_label:"Časová náročnost",note:"Výpočet je orientační a slouží pouze pro prezentaci možností výdělku obchodního zástupce.",top_performer_label:"TOP PERFORMER",top_performer_description:"Nejvyšší měsíční provize (H1 2026)",cta_text:"Mám zájem o spolupráci",cta_url:"https://example.com/cz-career",cta_target:"_blank"},
  ...
  EN:{locale:"en-GB",currency:"GBP",xlsxFilename:"Calculator_Data_EN.xlsx",
    texts:{title:"Extra work where you decide how much you earn",subtitle:"Calculate your estimated monthly commission based on the number of clients.",client_question:"How many clients do you serve per month?",badge:"INDICATIVE ESTIMATE",commission_title:"Commission estimate",monthly_commission:"monthly commission",clients_label:"Number of clients",time_label:"Time commitment",note:"The calculation is indicative and is intended only to demonstrate the earning potential of a field sales representative.",top_performer_label:"TOP PERFORMER",top_performer_description:"Best monthly commission (H1 2026)",cta_text:"I’m interested in cooperation",cta_url:"https://example.com/en-career",cta_target:"_blank"},
  ...
  RO:{locale:"ro-RO",currency:"RON",xlsxFilename:"Calculator_Data_RO.xlsx",
    texts:{title:"Lucrează suplimentar și decide singur cât câștigi",subtitle:"Calculează comisionul lunar estimativ în funcție de numărul de clienți.",client_question:"Câți clienți deservești pe lună?",badge:"CALCUL ESTIMATIV",commission_title:"Comision estimat",monthly_commission:"comision lunar",clients_label:"Număr de clienți",time_label:"Timp alocat",note:"Calculul este orientativ și are doar scopul de a prezenta posibilitățile de câștig ale unui reprezentant de vânzări pe teren.",top_performer_label:"TOP PERFORMER",top_performer_description:"Cel mai mare comision lunar (H1 2026)",cta_text:"Sunt interesat de colaborare",cta_url:"https://example.com/ro-career",cta_target:"_blank"},
...
const TEXT_FIELDS = [
  ["title","Headline",true],["subtitle","Subtitle",true],["client_question","Client question",false],
...
function parseRange(s){
  const n=nums(s);
  if(!n.length)return {min:0,max:999999};
  if(/[+]|more than|above|over|powyżej|peste|felett|více než/i.test(s))return {min:n[0]+(n.length===1?1:0),max:999999};
  if(n.length>1)return {min:n[0],max:n[1]};
  return {min:n[0],max:n[0]};
}
function bounds(){
  const rs=state.tiers.map(x=>parseRange(x.range));
  if(!rs.length)return {min:0,max:100};
  const min=Math.min(...rs.map(x=>x.min));
  const max=Math.max(...rs.map(x=>x.max===999999?x.min+1:x.max));
  return {min,max:Math.max(min+1,max)};
}
function tierFor(v){
  for(const t of state.tiers){const r=parseRange(t.range);if(v>=r.min&&v<=r.max)return t;}
  return state.tiers[state.tiers.length-1]||null;
}
function esc(v){
...
function renderPreview(reset=false){
  ...
  q("#pClientsLabel").textContent=t.clients_label;q("#pTimeLabel").textContent=t.time_label;q("#pNote").textContent=t.note;
  ...
  r.min=b.min;r.max=b.max;if(reset||+r.value<b.min||+r.value>b.max)r.value=Math.round((b.min+b.max)/2);
  const v=+r.value, tier=tierFor(v);
  q("#pScaleMin").textContent=b.min;q("#pScaleMax").textContent=`${b.max}+`;
  q("#pClients").textContent=v>=b.max?`${b.max}+`:v;q("#pTime").textContent=tier?.time||"";q("#pClientLabel").textContent=tier?.range||"";
  ...
  q("#sliderInfo").textContent=`Slider: ${b.min}–${b.max}`;q("#activeTierInfo").textContent=tier?`Active tier: ${tier.range}`:"No active tier";
  fitPreview();
}
...
function makeXlsx(){
  ...
  XLSX.utils.book_append_sheet(wb,XLSX.utils.aoa_to_sheet([["Client range","Time","Commission","Top performer"],...state.tiers.map(t=>[t.range,t.time,+t.commission||0,+t.top||0])]),"Tiers");
...
<div class="prov-calc__slider-row"><button class="prov-calc__step" type="button" data-step="minus">−</button><input class="prov-calc__range" type="range" min="${b.min}" max="${b.max}" value="${Math.round((b.min+b.max)/2)}" step="1"><button class="prov-calc__step" type="button" data-step="plus">+</button></div>
<div class="prov-calc__scale"><span>${b.min}</span><span>${b.max}+</span></div>
<div class="prov-calc__stats"><div class="prov-calc__stat"><div class="prov-calc__stat-title">${esc(t.clients_label)}</div><div class="prov-calc__stat-value" data-clients></div></div><div class="prov-calc__stat"><div class="prov-calc__stat-title">${esc(t.time_label)}</div><div class="prov-calc__stat-value" data-hours></div></div></div>
<div class="prov-calc__note">${esc(t.note)}</div></div>
...
<div class="prov-calc__features" data-benefits></div>
...
function parse(s){const n=String(s||'').match(/\\d+(?:[.,]\\d+)?/g)?.map(x=>Number(x.replace(',','.')))||[];if(!n.length)return{min:0,max:999999};if(/[+]|more than|above|over|powyżej|peste|felett|více než/i.test(s))return{min:n[0]+(n.length===1?1:0),max:999999};return n.length>1?{min:n[0],max:n[1]}:{min:n[0],max:n[0]};}
function tier(v){for(const t of TIERS){const r=parse(t.range);if(v>=r.min&&v<=r.max)return t}return TIERS[TIERS.length-1]}
